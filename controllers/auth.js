...
const signUp = async (req, res) => {
  const { name, email, phone, password, delivery } = req.body;
  ...
  try {
    ...
    if (matchUser) {
      console.log("User already exists:", matchUser);
      res.status(400).json({ message: 'User already exists' });
    } else {
      console.log("No user found. Proceeding to hash password.");
      
      const hashedPassword = await hashPassword(password);
      console.log("Password hashed:", hashedPassword);

      // Ensure that 'delivery.country.name' and 'delivery.country.code' are always provided as strings
      const newUser = new User({
        name,
        email,
        phone,
        ...
        delivery: {
          country: {
            name: typeof delivery?.country?.name === 'string' ? delivery?.country?.name : 'United States',  // Ensure it's a string
            code: typeof delivery?.country?.code === 'string' ? delivery?.country?.code : 'US'   // Ensure it's a string
          }
        }
      });
      ...
      console.log("Generating token for the new user...");
      ...
      console.log("Response sent with tokens.");
    }
  } catch (e) {
